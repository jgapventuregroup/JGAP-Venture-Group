  ...
  async function openLearningBook(){
    try{
      ...
      async function openChapter(id){
        ...
        if(error) throw error;
        e.innerHTML='<div class="muted">Chapter '+c.chapter_number+' • '+escapeHtml(c.part_title)+'</div><h2>'+escapeHtml(c.chapter_title)+'</h2><div class="panel" style="background:#f8fbff;margin-bottom:14px"><b>Chapter Overview</b><div style="margin-top:8px">'+(c.content_html||'<span class="muted">No overview yet.</span>')+'</div></div><div class="sectionTitle">Lessons</div><div id="lbRuntimeLessons">'+(lessons||[]).map(l=>'<button type="button" class="secondary" data-lb-lesson="'+l.id+'" style="display:block;width:100%;text-align:left;margin:5px 0;white-space:normal"><b>Lesson '+l.lesson_number+'</b> — '+escapeHtml(l.lesson_title)+' '+(l.completed?'✅':'')+'</button>').join('')+'</div><div id="lbRuntimeLesson" style="margin-top:14px"></div>';
